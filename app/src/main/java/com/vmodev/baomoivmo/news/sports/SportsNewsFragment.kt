...
import com.vmodev.baomoivmo.common.Constants.Companion.SPORT_NEWS_FRAGMENT_TAG
...
class SportsNewsFragment : Fragment(R.layout.fragment_sports_news) {
            ...
            when (response) {
                is Resource.Success -> {
                    ...
                    response.data?.let { newsResponse ->
                        ...
                        if(isLastPage){
                            rcvSportNews.setPadding(0,0,0,0)
                        }
                    }
                }
                is Resource.Error -> {
                    ...
                    response.message?.let { message ->
                        Log.e(SPORT_NEWS_FRAGMENT_TAG, message)
                    }
                }
                is Resource.Loading -> {
                    showProgressBar()
    ...
    private fun hideProgressBar() {
        paginationProgressBarSport.visibility = View.INVISIBLE
        isLoading = false
    }

    private fun showProgressBar() {
        paginationProgressBarSport.visibility = View.VISIBLE
        isLoading = true
    }
