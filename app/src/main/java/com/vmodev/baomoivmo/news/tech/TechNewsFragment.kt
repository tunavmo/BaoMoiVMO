package com.vmodev.baomoivmo.news.tech
...
class TechNewsFragment : Fragment(R.layout.fragment_tech_news) {
            ...
            when (response) {
                is Resource.Success -> {
                    ...
                    response.data?.let { newsResponse ->
                        ...
                        if(isLastPage){
                            rcvTechNews.setPadding(0,0,0,0)
                        }
                    }
                }
                is Resource.Error -> {
                    ...
                    response.message?.let { message ->
                        Log.e(TECH_NEWS_FRAGMENT_TAG, message)
                    }
                }
                is Resource.Loading -> {
                    showProgressBar()
    ...
    private fun hideProgressBar() {
        paginationProgressBarTech.visibility = View.INVISIBLE
        isLoading = false
    }

    private fun showProgressBar() {
        paginationProgressBarTech.visibility = View.VISIBLE
        isLoading = true
    }
    ...
    private fun setupRecyclerView() {
        ...
        rcvTechNews.apply {
            ...
            addOnScrollListener(this@TechNewsFragment.scrollListener)
        }
    }
}