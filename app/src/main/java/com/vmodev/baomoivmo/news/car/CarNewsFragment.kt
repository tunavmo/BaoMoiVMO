package com.vmodev.baomoivmo.news.car
...
import com.vmodev.baomoivmo.common.Constants.Companion.CAR_NEWS_FRAGMENT_TAG
...
class CarNewsFragment : Fragment(R.layout.fragment_car_news) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.carNews.observe(viewLifecycleOwner, Observer { response ->
            when (response) {
                is Resource.Success -> {
                    hideProgressBar()
                    response.data?.let { newsResponse ->
                        ...
                        isLastPage = viewModel.carNewsPage == totalPages
                        if(isLastPage){
                ...
                is Resource.Error -> {
                    ...
                    response.message?.let { message ->
                        Log.e(CAR_NEWS_FRAGMENT_TAG, message)
                    }
                }
                is Resource.Loading -> {
                    showProgressBar()
    ...
    private fun hideProgressBar() {
        paginationProgressBarCar.visibility = View.INVISIBLE
        isLoading = false
    }

    private fun showProgressBar() {
        paginationProgressBarCar.visibility = View.VISIBLE
        isLoading = true
    }
    ...
    private fun setupRecyclerView() {
        ...
        rcvCarNews.apply {
            ...
            addOnScrollListener(this@CarNewsFragment.scrollListener)
        }
    }
}